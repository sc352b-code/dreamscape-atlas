...
const WORLDS=[
  ...
  {id:'hearthlands',name:'The Hearthlands',cue:'Home · gardens · belonging',live:true,position:[-.18,-.02,.22],radius:.78,spin:.088,tilt:-.08,tint:0xffebc2,offset:.25,hearth:true,dreams:213},
...
// Portrait phones need a genuinely different solar-system composition rather than the
// desktop coordinates squeezed into a narrow viewport. These positions keep all five
// worlds visible while retaining asymmetry/depth.
const MOBILE_POSITIONS={
  littoral:[-.62,1.05,-.45],
  roadlands:[.65,.92,-.55],
  hearthlands:[0,.02,.20],
  institutional:[-.62,-1.13,-.60],
  river:[.65,-1.02,-.45],
};
...
function layoutFor(world){
  ...
  return {
    ...
    radius:radiusFor(world)*(mobile?.64:1),
    drift:mobile?.46:1,
  };
}
...
function mountTerritoryCosmos(){
  ...
  // Short, luminous 3D star corridor: a quick twinkling journey rather than a long tunnel.
  ...
  for(const world of WORLDS){
    ...
    scene.add(group);
    rendered.set(world.id,{world,radius:spec.radius,drift:spec.drift,group,mesh,atmosphere,material,atmosphereMaterial,base,phase:Math.random()*Math.PI*2});
  }
  ...
  function projectWorld(item){
    ...
    const radiusPx=Math.max(isPortraitMobile()?28:42,Math.abs(edge.x-center.x)*.5*innerWidth);
  ...
  function animate(now){
    ...
    if(!entering){
      ...
      const desired=new THREE.Vector3(target.x*.045,target.y*.045,THREE.MathUtils.lerp(7.55,1.22,p));
